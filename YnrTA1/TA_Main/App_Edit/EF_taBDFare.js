...
var script_taBDFare = {
  ACECountry1ID_Selected: function(sender, e) {
    ...
    var F_Country1ID_Display = $get(sender._element.id + '_Display');
  ...
  validate_FK_TA_BillDetails_Country1ID: function(o,Prefix) {
    var value = o.id;
    var Country1ID = $get(Prefix + 'Country1ID');
    if(Country1ID.value==''){
      if(this.validated_FK_TA_BillDetails_Country1ID_main){
        var o_d = $get(Prefix + 'Country1ID' + '_Display');
  ...
  validated_FK_TA_BillDetails_Country1ID: function(result) {
    var p = result.split('|');
    var o = $get(p[1]);
    if(script_taBDFare.validated_FK_TA_BillDetails_Country1ID_main){
  ...
  ACECountry2ID_Selected: function(sender, e) {
    ...
    var F_Country2ID_Display = $get(sender._element.id + '_Display');
    var retval = e.get_value();
    var p = retval.split('|');
  ...
  validate_FK_TA_BillDetails_Country2ID: function(o,Prefix) {
    var value = o.id;
    var Country2ID = $get(Prefix + 'Country2ID');
    if(Country2ID.value==''){
      if(this.validated_FK_TA_BillDetails_Country2ID_main){
        var o_d = $get(Prefix + 'Country2ID' + '_Display');
        try{o_d.innerHTML = '';}catch(ex){}
      }
      return true;
    }
  ...
  validated_FK_TA_BillDetails_Country2ID: function(result) {
    var p = result.split('|');
    var o = $get(p[1]);
    if(script_taBDFare.validated_FK_TA_BillDetails_Country2ID_main){
      var o_d = $get(p[1]+'_Display');
      try{o_d.innerHTML = p[2];}catch(ex){}
    }
    o.style.backgroundImage  = 'none';
  ...
  ACETABillNo_Selected: function(sender, e) {
    ...
    validated_FK_TA_BillDetails_ProjectID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_taBDFare.validated_FK_TA_BillDetails_ProjectID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
