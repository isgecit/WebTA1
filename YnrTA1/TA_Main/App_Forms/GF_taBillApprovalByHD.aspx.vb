...
Partial Class GF_taBillApprovalByHD
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/TA_Main/App_Display/DF_taBillApprovalByHD.aspx"
  Protected Sub Info_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
    Dim oBut As ImageButton = CType(sender, ImageButton)
    Dim aVal() As String = oBut.CommandArgument.ToString.Split(",".ToCharArray)
    Dim RedirectUrl As String = _InfoUrl & "?TABillNo=" & aVal(0)
    Response.Redirect(RedirectUrl)
  End Sub
  Protected Sub GVtaBillApprovalByHD_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVtaBillApprovalByHD.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim TABillNo As Int32 = GVtaBillApprovalByHD.DataKeys(e.CommandArgument).Values("TABillNo")
        Dim RedirectUrl As String = TBLtaBillApprovalByHD.EditUrl & "?TABillNo=" & TABillNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim ApprovalRemarks As String = CType(GVtaBillApprovalByHD.Rows(e.CommandArgument).FindControl("F_ApprovalRemarks"), TextBox).Text
        Dim TABillNo As Int32 = GVtaBillApprovalByHD.DataKeys(e.CommandArgument).Values("TABillNo")
        SIS.TA.taBillApprovalByHD.ApproveWF(TABillNo, ApprovalRemarks)
        GVtaBillApprovalByHD.DataBind()
      Catch ex As Exception
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim ApprovalRemarks As String = CType(GVtaBillApprovalByHD.Rows(e.CommandArgument).FindControl("F_ApprovalRemarks"), TextBox).Text
        Dim TABillNo As Int32 = GVtaBillApprovalByHD.DataKeys(e.CommandArgument).Values("TABillNo")
        SIS.TA.taBillApprovalByHD.RejectWF(TABillNo, ApprovalRemarks)
        GVtaBillApprovalByHD.DataBind()
      Catch ex As Exception
        Dim str As String = New JavaScriptSerializer().Serialize(ex.Message)
        Dim script As String = String.Format("alert({0});", str)
    ...
    End If
  End Sub
  Protected Sub GVtaBillApprovalByHD_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVtaBillApprovalByHD.Init
    DataClassName = "GtaBillApprovalByHD"
    SetGridView = GVtaBillApprovalByHD
  End Sub
  Protected Sub TBLtaBillApprovalByHD_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLtaBillApprovalByHD.Init
    SetToolBar = TBLtaBillApprovalByHD
  End Sub
