<%@ Page Language="VB" MasterPageFile="~/MasterPage.master" AutoEventWireup="False" CodeFile="GF_taBHAll.aspx.vb" Inherits="GF_taBHAll" title="Maintain List: Travel Expense Statements" %>
  ...
  <ContentTemplate>
    ...
    <script type="text/javascript">
      ...
      function print_report(o) {
        pcnt = pcnt + 1;
        var nam = 'wTask' + pcnt;
        var url = self.location.href.replace('App_Forms/GF_taBHAll','App_Print/RP_taBH');
    ...
    <asp:GridView ID="GVtaBH" SkinID="gv_silver" runat="server" DataSourceID="ODStaBH" DataKeyNames="TABillNo">
      <Columns>
        ...
        <asp:TemplateField HeaderText="Employee ID" SortExpression="HRM_Employees5_EmployeeName">
          <ItemTemplate>
             <asp:LinkButton ID="L_EmployeeID" runat="server" ForeColor='<%# EVal("ForeColor") %>' ToolTip='<%# EVal("EmployeeID") %>' Text='<%# Eval("HRM_Employees5_EmployeeName") %>' CommandName="lgempedit"  CommandArgument='<%# Container.DataItemIndex %>'></asp:LinkButton>
          </ItemTemplate>
        ...
        <asp:TemplateField HeaderText="Bill Status" SortExpression="TA_BillStates16_Description">
          <ItemTemplate>
             <LGM:LC_taBillStates 
               runat="server" 
               id="LC_BillStatusID"  
               SelectedValue='<%# Eval("BillStatusID") %>'
               ...
               CssClass="myddl"
               Runat="Server" />
               <asp:ImageButton ID="cmdSaveStatus" runat="server" Visible='<%# EVal("Visible") %>' Enabled='<%# EVal("Enable") %>' AlternateText="Save" ToolTip="Update Bill Status." SkinID="save" CommandName="lgStatusSave" CommandArgument='<%# Container.DataItemIndex %>' />
          </ItemTemplate>
          <ItemStyle CssClass="alignCenter" />
          <HeaderStyle Width="100px" />
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="DEL">
          <ItemTemplate>
            <asp:ImageButton ID="cmdDel"  runat="server" Visible='<%# EVal("Visible") %>' Enabled='<%# EVal("Enable") %>' AlternateText="Delete" ToolTip="Delete the record." SkinID="delete" OnClientClick="return confirm('Delete TA BILL ???')" CommandName="lgDelete" CommandArgument='<%# Container.DataItemIndex %>' />
          </ItemTemplate>
          <ItemStyle CssClass="alignCenter" />
          <HeaderStyle HorizontalAlign="Center" Width="30px" />
        </asp:TemplateField>
    ...
    <asp:ObjectDataSource 
      ...
      SelectMethod = "UZ_taBHAllSelectList"
      TypeName = "SIS.TA.taBH"
      SelectCountMethod = "taBHAllSelectCount"
