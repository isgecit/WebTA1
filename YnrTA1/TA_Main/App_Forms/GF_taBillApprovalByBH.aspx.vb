...
Partial Class GF_taBillApprovalByBH
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/TA_Main/App_Display/DF_taBillApprovalByBH.aspx"
  ...
  End Sub
  Protected Sub GVtaBillApprovalByBH_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVtaBillApprovalByBH.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim TABillNo As Int32 = GVtaBillApprovalByBH.DataKeys(e.CommandArgument).Values("TABillNo")
        Dim RedirectUrl As String = TBLtaBillApprovalByBH.EditUrl & "?TABillNo=" & TABillNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim CCRemarks As String = CType(GVtaBillApprovalByBH.Rows(e.CommandArgument).FindControl("F_CCRemarks"), TextBox).Text
        Dim TABillNo As Int32 = GVtaBillApprovalByBH.DataKeys(e.CommandArgument).Values("TABillNo")
        SIS.TA.taBillApprovalByBH.ApproveWF(TABillNo, CCRemarks)
        GVtaBillApprovalByBH.DataBind()
      Catch ex As Exception
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim CCRemarks As String = CType(GVtaBillApprovalByBH.Rows(e.CommandArgument).FindControl("F_CCRemarks"), TextBox).Text
        Dim TABillNo As Int32 = GVtaBillApprovalByBH.DataKeys(e.CommandArgument).Values("TABillNo")
        SIS.TA.taBillApprovalByBH.RejectWF(TABillNo, CCRemarks)
        GVtaBillApprovalByBH.DataBind()
      Catch ex As Exception
        Dim str As String = New JavaScriptSerializer().Serialize(ex.Message)
        Dim script As String = String.Format("alert({0});", str)
    ...
    End If
  End Sub
  Protected Sub GVtaBillApprovalByBH_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVtaBillApprovalByBH.Init
    DataClassName = "GtaBillApprovalByBH"
    SetGridView = GVtaBillApprovalByBH
  End Sub
  Protected Sub TBLtaBillApprovalByBH_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLtaBillApprovalByBH.Init
    SetToolBar = TBLtaBillApprovalByBH
  End Sub
