Partial Class GF_eitlClosedPO
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/EITL_Main/App_Display/DF_eitlClosedPO.aspx"
  ...
  End Sub
  Protected Sub GVeitlClosedPO_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVeitlClosedPO.RowCommand
		If e.CommandName.ToLower = "lgedit".ToLower Then
			Try
				Dim SerialNo As Int32 = GVeitlClosedPO.DataKeys(e.CommandArgument).Values("SerialNo")  
				Dim RedirectUrl As String = TBLeitlClosedPO.EditUrl & "?SerialNo=" & SerialNo
				Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "ApproveWF".ToLower Then
      Try
        Dim Reason As String = CType(GVeitlClosedPO.Rows(e.CommandArgument).FindControl("F_Reason"), TextBox).Text
        Dim SerialNo As Int32 = GVeitlClosedPO.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.EITL.eitlClosedPO.ApproveWF(SerialNo, Reason)
        GVeitlClosedPO.EditIndex = -1
        GVeitlClosedPO.DataBind()
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVeitlClosedPO_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVeitlClosedPO.Init
    DataClassName = "GeitlClosedPO"
    SetGridView = GVeitlClosedPO
  End Sub
  Protected Sub TBLeitlClosedPO_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLeitlClosedPO.Init
    SetToolBar = TBLeitlClosedPO
  End Sub
