...
Partial Class GF_eitlPOList
  Inherits SIS.SYS.GridBase
  Protected Sub GVeitlPOList_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVeitlPOList.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim SerialNo As Int32 = GVeitlPOList.DataKeys(e.CommandArgument).Values("SerialNo")
        Dim RedirectUrl As String = TBLeitlPOList.EditUrl & "?SerialNo=" & SerialNo
    ...
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVeitlPOList.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.EITL.eitlPOList.InitiateWF(SerialNo)
        GVeitlPOList.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", script, True)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVeitlPOList.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.EITL.eitlPOList.ApproveWF(SerialNo)
        GVeitlPOList.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
    ...
    If e.CommandName.ToLower = "completewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVeitlPOList.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.EITL.eitlPOList.CompleteWF(SerialNo)
        GVeitlPOList.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
    ...
    End If
  End Sub
  Protected Sub GVeitlPOList_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVeitlPOList.Init
    DataClassName = "GeitlPOList"
    SetGridView = GVeitlPOList
  End Sub
