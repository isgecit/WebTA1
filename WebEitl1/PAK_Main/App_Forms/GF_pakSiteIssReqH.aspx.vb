...
Partial Class GF_pakSiteIssReqH
  ...
  End Sub
  Protected Sub GVpakSiteIssReqH_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpakSiteIssReqH.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ProjectID As String = GVpakSiteIssReqH.DataKeys(e.CommandArgument).Values("ProjectID")  
        Dim IssueNo As Int32 = GVpakSiteIssReqH.DataKeys(e.CommandArgument).Values("IssueNo")  
        Dim RedirectUrl As String = TBLpakSiteIssReqH.EditUrl & "?ProjectID=" & ProjectID & "&IssueNo=" & IssueNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim ProjectID As String = GVpakSiteIssReqH.DataKeys(e.CommandArgument).Values("ProjectID")  
        Dim IssueNo As Int32 = GVpakSiteIssReqH.DataKeys(e.CommandArgument).Values("IssueNo")  
        SIS.PAK.pakSiteIssReqH.InitiateWF(ProjectID, IssueNo)
        GVpakSiteIssReqH.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpakSiteIssReqH_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpakSiteIssReqH.Init
    DataClassName = "GpakSiteIssReqH"
    SetGridView = GVpakSiteIssReqH
  End Sub
