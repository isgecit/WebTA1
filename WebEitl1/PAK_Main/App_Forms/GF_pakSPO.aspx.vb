...
Partial Class GF_pakSPO
  Inherits SIS.SYS.GridBase
  Protected Sub GVpakSPO_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpakSPO.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSPO.DataKeys(e.CommandArgument).Values("SerialNo")
        Dim RedirectUrl As String = TBLpakSPO.EditUrl & "?SerialNo=" & SerialNo
    ...
    If e.CommandName.ToLower = "initiatewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSPO.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.PAK.pakSPO.InitiateWF(SerialNo)
        GVpakSPO.DataBind()
      Catch ex As Exception
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSPO.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.PAK.pakSPO.ApproveWF(SerialNo)
        GVpakSPO.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSPO.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.PAK.pakSPO.RejectWF(SerialNo)
        GVpakSPO.DataBind()
      Catch ex As Exception
      End Try
    End If
    If e.CommandName.ToLower = "completewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSPO.DataKeys(e.CommandArgument).Values("SerialNo")
        SIS.PAK.pakSPO.CompleteWF(SerialNo)
        GVpakSPO.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
    ...
    End If
  End Sub
  Protected Sub GVpakSPO_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpakSPO.Init
    DataClassName = "GpakSPO"
    SetGridView = GVpakSPO
  End Sub
