...
Partial Class GF_pakSiteMtlIssDL
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/PAK_Main/App_Display/DF_pakSiteMtlIssDL.aspx"
  Protected Sub Info_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
    Dim oBut As ImageButton = CType(sender, ImageButton)
    Dim aVal() As String = oBut.CommandArgument.ToString.Split(",".ToCharArray)
    Dim RedirectUrl As String = _InfoUrl  & "?ProjectID=" & aVal(0) & "&IssueNo=" & aVal(1) & "&IssueSrNo=" & aVal(2) & "&IssueSrLNo=" & aVal(3)
  ...
  Protected Sub GVpakSiteMtlIssDL_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpakSiteMtlIssDL.Init
    DataClassName = "GpakSiteMtlIssDL"
    SetGridView = GVpakSiteMtlIssDL
  End Sub
  ...
  End Function
  Protected Sub F_IssueSrNo_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles F_IssueSrNo.TextChanged
    Session("F_IssueSrNo") = F_IssueSrNo.Text
    Session("F_IssueSrNo_Display") = F_IssueSrNo_Display.Text
    InitGridPage()
  End Sub
  <System.Web.Services.WebMethod()> _
  <System.Web.Script.Services.ScriptMethod()> _
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim strScriptPopulatingIssueNo As String = "<script type=""text/javascript""> " & _
      ...
      End If
    F_IssueSrNo_Display.Text = String.Empty
    If Not Session("F_IssueSrNo_Display") Is Nothing Then
      If Session("F_IssueSrNo_Display") <> String.Empty Then
        F_IssueSrNo_Display.Text = Session("F_IssueSrNo_Display")
      End If
    End If
    F_IssueSrNo.Text = String.Empty
    If Not Session("F_IssueSrNo") Is Nothing Then
      If Session("F_IssueSrNo") <> String.Empty Then
        F_IssueSrNo.Text = Session("F_IssueSrNo")
      End If
    End If
    Dim strScriptIssueSrNo As String = "<script type=""text/javascript""> " & _
      "function ACEIssueSrNo_Selected(sender, e) {" & _
      "  var F_IssueSrNo = $get('" & F_IssueSrNo.ClientID & "');" & _
      "  var F_IssueSrNo_Display = $get('" & F_IssueSrNo_Display.ClientID & "');" & _
      "  var retval = e.get_value();" & _
      "  var p = retval.split('|');" & _
      "  F_IssueSrNo.value = p[2];" & _
      "  F_IssueSrNo_Display.innerHTML = e.get_text();" & _
      "}" & _
      "</script>"
      If Not Page.ClientScript.IsClientScriptBlockRegistered("F_IssueSrNo") Then
        Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "F_IssueSrNo", strScriptIssueSrNo)
      End If
    Dim strScriptPopulatingIssueSrNo As String = "<script type=""text/javascript""> " & _
      "function ACEIssueSrNo_Populating(o,e) {" & _
      "  var p = $get('" & F_IssueSrNo.ClientID & "');" & _
      "  p.style.backgroundImage  = 'url(../../images/loader.gif)';" & _
      "  p.style.backgroundRepeat= 'no-repeat';" & _
      "  p.style.backgroundPosition = 'right';" & _
      "  o._contextKey = '';" & _
      "}" & _
      "function ACEIssueSrNo_Populated(o,e) {" & _
      "  var p = $get('" & F_IssueSrNo.ClientID & "');" & _
      "  p.style.backgroundImage  = 'none';" & _
      "}" & _
      "</script>"
      If Not Page.ClientScript.IsClientScriptBlockRegistered("F_IssueSrNoPopulating") Then
        Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "F_IssueSrNoPopulating", strScriptPopulatingIssueSrNo)
      End If
    ...
    Dim validateScriptIssueNo As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_PAK_SiteIssueDLocation_IssueNo_main = true;" & _
      "    validate_FK_PAK_SiteIssueDLocation_IssueNo(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptIssueSrNo As String = "<script type=""text/javascript"">" & _
      "  function validate_IssueSrNo(o) {" & _
      "    validated_FK_PAK_SiteIssueDLocation_IssueSrNo_main = true;" & _
      "    validate_FK_PAK_SiteIssueDLocation_IssueSrNo(o);" & _
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateIssueSrNo") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateIssueSrNo", validateScriptIssueSrNo)
    End If
    Dim validateScriptProjectID As String = "<script type=""text/javascript"">" & _
      "  function validate_ProjectID(o) {" & _
      "    validated_FK_PAK_SiteIssueDLocation_ProjectID_main = true;" & _
      "    validate_FK_PAK_SiteIssueDLocation_ProjectID(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_PAK_SiteIssueDLocation_ProjectID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_PAK_SiteIssueDLocation_ProjectID(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectID = $get('" & F_ProjectID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectID.value==''){" & _
      "      if(validated_FK_PAK_SiteIssueDLocation_ProjectID.main){" & _
      ...
      "    PageMethods.validate_FK_PAK_SiteIssueDLocation_ProjectID(value, validated_FK_PAK_SiteIssueDLocation_ProjectID);" & _
      "  }" & _
      "  validated_FK_PAK_SiteIssueDLocation_ProjectID_main = false;" & _
      "  function validated_FK_PAK_SiteIssueDLocation_ProjectID(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_PAK_SiteIssueDLocation_ProjectID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_PAK_SiteIssueDLocation_ProjectID", validateScriptFK_PAK_SiteIssueDLocation_ProjectID)
    End If
    Dim validateScriptFK_PAK_SiteIssueDLocation_IssueSrNo As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_PAK_SiteIssueDLocation_IssueSrNo(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectID = $get('" & F_ProjectID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectID.value==''){" & _
      "      if(validated_FK_PAK_SiteIssueDLocation_IssueSrNo.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    }catch(ex){}" & _
      "    var IssueSrNo = $get('" & F_IssueSrNo.ClientID & "');" & _
      "    try{" & _
      "    if(IssueSrNo.value==''){" & _
      "      if(validated_FK_PAK_SiteIssueDLocation_IssueSrNo.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      "    value = value + ',' + IssueSrNo.value ;" & _
      "    }catch(ex){}" & _
      "    o.style.backgroundImage  = 'url(../../images/pkloader.gif)';" & _
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_PAK_SiteIssueDLocation_IssueSrNo(value, validated_FK_PAK_SiteIssueDLocation_IssueSrNo);" & _
      "  }" & _
      "  validated_FK_PAK_SiteIssueDLocation_IssueSrNo_main = false;" & _
      "  function validated_FK_PAK_SiteIssueDLocation_IssueSrNo(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_PAK_SiteIssueDLocation_IssueSrNo") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_PAK_SiteIssueDLocation_IssueSrNo", validateScriptFK_PAK_SiteIssueDLocation_IssueSrNo)
    End If
    Dim validateScriptFK_PAK_SiteIssueDLocation_IssueNo As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_PAK_SiteIssueDLocation_IssueNo(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectID = $get('" & F_ProjectID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectID.value==''){" & _
      "      if(validated_FK_PAK_SiteIssueDLocation_IssueNo.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_PAK_SiteIssueDLocation_IssueNo(value, validated_FK_PAK_SiteIssueDLocation_IssueNo);" & _
      "  }" & _
      "  validated_FK_PAK_SiteIssueDLocation_IssueNo_main = false;" & _
      "  function validated_FK_PAK_SiteIssueDLocation_IssueNo(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_PAK_SiteIssueDLocation_IssueNo") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_PAK_SiteIssueDLocation_IssueNo", validateScriptFK_PAK_SiteIssueDLocation_IssueNo)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
