...
var script_costCostSheet = {
    ...
    validate_ProjectGroupID: function(sender) {
      ...
      this.validated_FK_COST_CostSheet_ProjectGroupID_main = true;
      this.validate_FK_COST_CostSheet_ProjectGroupID(sender,Prefix);
      },
    ...
    validate_FK_COST_CostSheet_ProjectGroupID: function(o,Prefix) {
      var value = o.id;
      var ProjectGroupID = $get(Prefix + 'ProjectGroupID');
      if(ProjectGroupID.value==''){
        if(this.validated_FK_COST_CostSheet_ProjectGroupID_main){
      ...
      value = value + ',' + ProjectGroupID.value ;
        ...
        o.style.backgroundRepeat= 'no-repeat';
        o.style.backgroundPosition = 'right';
        PageMethods.validate_FK_COST_CostSheet_ProjectGroupID(value, this.validated_FK_COST_CostSheet_ProjectGroupID);
      },
    validated_FK_COST_CostSheet_ProjectGroupID_main: false,
    validated_FK_COST_CostSheet_ProjectGroupID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_costCostSheet.validated_FK_COST_CostSheet_ProjectGroupID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
