Partial Class GF_costCostSheetLiability
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/COST_Main/App_Display/DF_costCostSheetLiability.aspx"
  Protected Sub Info_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
    Dim oBut As ImageButton = CType(sender, ImageButton)
    Dim aVal() As String = oBut.CommandArgument.ToString.Split(",".ToCharArray)
    Dim RedirectUrl As String = _InfoUrl  & "?ProjectGroupID=" & aVal(0) & "&FinYear=" & aVal(1) & "&Quarter=" & aVal(2) & "&Revision=" & aVal(3) & "&ProjectID=" & aVal(4) & "&GLCode=" & aVal(5) & "&AdjustmentSerialNo=" & aVal(6) & "&AdjustmentCredit=" & aVal(7)
    Response.Redirect(RedirectUrl)
  End Sub
  Protected Sub GVcostCostSheetLiability_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVcostCostSheetLiability.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ProjectGroupID As Int32 = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("ProjectGroupID")  
        Dim FinYear As Int32 = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("FinYear")  
        Dim Quarter As Int32 = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("Quarter")  
        Dim Revision As Int32 = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("Revision")  
        Dim ProjectID As String = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("ProjectID")  
        Dim GLCode As String = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("GLCode")  
        Dim AdjustmentSerialNo As Int32 = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("AdjustmentSerialNo")  
        Dim AdjustmentCredit As Boolean = GVcostCostSheetLiability.DataKeys(e.CommandArgument).Values("AdjustmentCredit")  
        Dim RedirectUrl As String = TBLcostCostSheetLiability.EditUrl & "?ProjectGroupID=" & ProjectGroupID & "&FinYear=" & FinYear & "&Quarter=" & Quarter & "&Revision=" & Revision & "&ProjectID=" & ProjectID & "&GLCode=" & GLCode & "&AdjustmentSerialNo=" & AdjustmentSerialNo & "&AdjustmentCredit=" & AdjustmentCredit
        Response.Redirect(RedirectUrl)
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVcostCostSheetLiability_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVcostCostSheetLiability.Init
    DataClassName = "GcostCostSheetLiability"
    SetGridView = GVcostCostSheetLiability
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim validateScriptProjectGroupID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CostSheetLiability_ProjectGroupID_main = true;" & _
      "    validate_FK_COST_CostSheetLiability_ProjectGroupID(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptFinYear As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CostSheetLiability_FinYear_main = true;" & _
      "    validate_FK_COST_CostSheetLiability_FinYear(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptQuarter As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CostSheetLiability_Quarter_main = true;" & _
      "    validate_FK_COST_CostSheetLiability_Quarter(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_COST_CostSheetLiability_FinYear As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CostSheetLiability_FinYear(o) {" & _
      "    var value = o.id;" & _
      "    var FinYear = $get('" & F_FinYear.ClientID & "');" & _
      "    try{" & _
      "    if(FinYear.value==''){" & _
      "      if(validated_FK_COST_CostSheetLiability_FinYear.main){" & _
      ...
      "    PageMethods.validate_FK_COST_CostSheetLiability_FinYear(value, validated_FK_COST_CostSheetLiability_FinYear);" & _
      "  }" & _
      "  validated_FK_COST_CostSheetLiability_FinYear_main = false;" & _
      "  function validated_FK_COST_CostSheetLiability_FinYear(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CostSheetLiability_FinYear") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CostSheetLiability_FinYear", validateScriptFK_COST_CostSheetLiability_FinYear)
    End If
    Dim validateScriptFK_COST_CostSheetLiability_ProjectGroupID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CostSheetLiability_ProjectGroupID(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectGroupID = $get('" & F_ProjectGroupID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectGroupID.value==''){" & _
      "      if(validated_FK_COST_CostSheetLiability_ProjectGroupID.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_CostSheetLiability_ProjectGroupID(value, validated_FK_COST_CostSheetLiability_ProjectGroupID);" & _
      "  }" & _
      "  validated_FK_COST_CostSheetLiability_ProjectGroupID_main = false;" & _
      "  function validated_FK_COST_CostSheetLiability_ProjectGroupID(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CostSheetLiability_ProjectGroupID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CostSheetLiability_ProjectGroupID", validateScriptFK_COST_CostSheetLiability_ProjectGroupID)
    End If
    Dim validateScriptFK_COST_CostSheetLiability_Quarter As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CostSheetLiability_Quarter(o) {" & _
      "    var value = o.id;" & _
      "    var Quarter = $get('" & F_Quarter.ClientID & "');" & _
      "    try{" & _
      "    if(Quarter.value==''){" & _
      "      if(validated_FK_COST_CostSheetLiability_Quarter.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_CostSheetLiability_Quarter(value, validated_FK_COST_CostSheetLiability_Quarter);" & _
      "  }" & _
      "  validated_FK_COST_CostSheetLiability_Quarter_main = false;" & _
      "  function validated_FK_COST_CostSheetLiability_Quarter(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CostSheetLiability_Quarter") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CostSheetLiability_Quarter", validateScriptFK_COST_CostSheetLiability_Quarter)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_COST_CostSheetLiability_FinYear(ByVal value As String) As String
  ...
  End Function
  <System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_COST_CostSheetLiability_ProjectGroupID(ByVal value As String) As String
    Dim aVal() As String = value.Split(",".ToCharArray)
    Dim mRet As String="0|" & aVal(0)
  ...
  End Function
  <System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_COST_CostSheetLiability_Quarter(ByVal value As String) As String
    Dim aVal() As String = value.Split(",".ToCharArray)
    Dim mRet As String="0|" & aVal(0)
