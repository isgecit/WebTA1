Partial Class GF_costCostSheetData
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/COST_Main/App_Display/DF_costCostSheetData.aspx"
  ...
  Protected Sub GVcostCostSheetData_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVcostCostSheetData.Init
    DataClassName = "GcostCostSheetData"
    SetGridView = GVcostCostSheetData
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim validateScriptProjectGroupID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CostSheetData_ProjectGroupID_main = true;" & _
      "    validate_FK_COST_CostSheetData_ProjectGroupID(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptFinYear As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CostSheetData_FinYear_main = true;" & _
      "    validate_FK_COST_CostSheetData_FinYear(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptQuarter As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CostSheetData_Quarter_main = true;" & _
      "    validate_FK_COST_CostSheetData_Quarter(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_COST_CostSheetData_FinYear As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CostSheetData_FinYear(o) {" & _
      "    var value = o.id;" & _
      "    var FinYear = $get('" & F_FinYear.ClientID & "');" & _
      "    try{" & _
      "    if(FinYear.value==''){" & _
      "      if(validated_FK_COST_CostSheetData_FinYear.main){" & _
      ...
      "    PageMethods.validate_FK_COST_CostSheetData_FinYear(value, validated_FK_COST_CostSheetData_FinYear);" & _
      "  }" & _
      "  validated_FK_COST_CostSheetData_FinYear_main = false;" & _
      "  function validated_FK_COST_CostSheetData_FinYear(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CostSheetData_FinYear") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CostSheetData_FinYear", validateScriptFK_COST_CostSheetData_FinYear)
    End If
    Dim validateScriptFK_COST_CostSheetData_ProjectGroupID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CostSheetData_ProjectGroupID(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectGroupID = $get('" & F_ProjectGroupID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectGroupID.value==''){" & _
      "      if(validated_FK_COST_CostSheetData_ProjectGroupID.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_CostSheetData_ProjectGroupID(value, validated_FK_COST_CostSheetData_ProjectGroupID);" & _
      "  }" & _
      "  validated_FK_COST_CostSheetData_ProjectGroupID_main = false;" & _
      "  function validated_FK_COST_CostSheetData_ProjectGroupID(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CostSheetData_ProjectGroupID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CostSheetData_ProjectGroupID", validateScriptFK_COST_CostSheetData_ProjectGroupID)
    End If
    Dim validateScriptFK_COST_CostSheetData_Quarter As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CostSheetData_Quarter(o) {" & _
      "    var value = o.id;" & _
      "    var Quarter = $get('" & F_Quarter.ClientID & "');" & _
      "    try{" & _
      "    if(Quarter.value==''){" & _
      "      if(validated_FK_COST_CostSheetData_Quarter.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_CostSheetData_Quarter(value, validated_FK_COST_CostSheetData_Quarter);" & _
      "  }" & _
      "  validated_FK_COST_CostSheetData_Quarter_main = false;" & _
      "  function validated_FK_COST_CostSheetData_Quarter(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CostSheetData_Quarter") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CostSheetData_Quarter", validateScriptFK_COST_CostSheetData_Quarter)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_COST_CostSheetData_FinYear(ByVal value As String) As String
  ...
  End Function
  <System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_COST_CostSheetData_ProjectGroupID(ByVal value As String) As String
    Dim aVal() As String = value.Split(",".ToCharArray)
    Dim mRet As String="0|" & aVal(0)
  ...
  End Function
  <System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_COST_CostSheetData_Quarter(ByVal value As String) As String
    Dim aVal() As String = value.Split(",".ToCharArray)
    Dim mRet As String="0|" & aVal(0)
