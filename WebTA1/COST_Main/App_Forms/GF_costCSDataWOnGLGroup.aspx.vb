Partial Class GF_costCSDataWOnGLGroup
  ...
  End Sub
  Protected Sub GVcostCSDataWOnGLGroup_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVcostCSDataWOnGLGroup.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ProjectGroupID As Int32 = GVcostCSDataWOnGLGroup.DataKeys(e.CommandArgument).Values("ProjectGroupID")  
        Dim FinYear As Int32 = GVcostCSDataWOnGLGroup.DataKeys(e.CommandArgument).Values("FinYear")  
        Dim Quarter As Int32 = GVcostCSDataWOnGLGroup.DataKeys(e.CommandArgument).Values("Quarter")  
        Dim Revision As Int32 = GVcostCSDataWOnGLGroup.DataKeys(e.CommandArgument).Values("Revision")  
        Dim WorkOrderTypeID As Int32 = GVcostCSDataWOnGLGroup.DataKeys(e.CommandArgument).Values("WorkOrderTypeID")  
        Dim GLGroupID As Int32 = GVcostCSDataWOnGLGroup.DataKeys(e.CommandArgument).Values("GLGroupID")  
        Dim RedirectUrl As String = TBLcostCSDataWOnGLGroup.EditUrl & "?ProjectGroupID=" & ProjectGroupID & "&FinYear=" & FinYear & "&Quarter=" & Quarter & "&Revision=" & Revision & "&WorkOrderTypeID=" & WorkOrderTypeID & "&GLGroupID=" & GLGroupID
        Response.Redirect(RedirectUrl)
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVcostCSDataWOnGLGroup_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVcostCSDataWOnGLGroup.Init
    DataClassName = "GcostCSDataWOnGLGroup"
    SetGridView = GVcostCSDataWOnGLGroup
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim validateScriptProjectGroupID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CSDataWOnGLGroup_ProjectGroupID_main = true;" & _
      "    validate_FK_COST_CSDataWOnGLGroup_ProjectGroupID(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptFinYear As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CSDataWOnGLGroup_FinYear_main = true;" & _
      "    validate_FK_COST_CSDataWOnGLGroup_FinYear(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptQuarter As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_CSDataWOnGLGroup_Quarter_main = true;" & _
      "    validate_FK_COST_CSDataWOnGLGroup_Quarter(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_COST_CSDataWOnGLGroup_FinYear As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CSDataWOnGLGroup_FinYear(o) {" & _
      "    var value = o.id;" & _
      "    var FinYear = $get('" & F_FinYear.ClientID & "');" & _
      "    try{" & _
      "    if(FinYear.value==''){" & _
      "      if(validated_FK_COST_CSDataWOnGLGroup_FinYear.main){" & _
      ...
      "    PageMethods.validate_FK_COST_CSDataWOnGLGroup_FinYear(value, validated_FK_COST_CSDataWOnGLGroup_FinYear);" & _
      "  }" & _
      "  validated_FK_COST_CSDataWOnGLGroup_FinYear_main = false;" & _
      "  function validated_FK_COST_CSDataWOnGLGroup_FinYear(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CSDataWOnGLGroup_FinYear") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CSDataWOnGLGroup_FinYear", validateScriptFK_COST_CSDataWOnGLGroup_FinYear)
    End If
    Dim validateScriptFK_COST_CSDataWOnGLGroup_ProjectGroupID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CSDataWOnGLGroup_ProjectGroupID(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectGroupID = $get('" & F_ProjectGroupID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectGroupID.value==''){" & _
      "      if(validated_FK_COST_CSDataWOnGLGroup_ProjectGroupID.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_CSDataWOnGLGroup_ProjectGroupID(value, validated_FK_COST_CSDataWOnGLGroup_ProjectGroupID);" & _
      "  }" & _
      "  validated_FK_COST_CSDataWOnGLGroup_ProjectGroupID_main = false;" & _
      "  function validated_FK_COST_CSDataWOnGLGroup_ProjectGroupID(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CSDataWOnGLGroup_ProjectGroupID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CSDataWOnGLGroup_ProjectGroupID", validateScriptFK_COST_CSDataWOnGLGroup_ProjectGroupID)
    End If
    Dim validateScriptFK_COST_CSDataWOnGLGroup_Quarter As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_CSDataWOnGLGroup_Quarter(o) {" & _
      "    var value = o.id;" & _
      "    var Quarter = $get('" & F_Quarter.ClientID & "');" & _
      "    try{" & _
      "    if(Quarter.value==''){" & _
      "      if(validated_FK_COST_CSDataWOnGLGroup_Quarter.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_CSDataWOnGLGroup_Quarter(value, validated_FK_COST_CSDataWOnGLGroup_Quarter);" & _
      "  }" & _
      "  validated_FK_COST_CSDataWOnGLGroup_Quarter_main = false;" & _
      "  function validated_FK_COST_CSDataWOnGLGroup_Quarter(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_CSDataWOnGLGroup_Quarter") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_CSDataWOnGLGroup_Quarter", validateScriptFK_COST_CSDataWOnGLGroup_Quarter)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
