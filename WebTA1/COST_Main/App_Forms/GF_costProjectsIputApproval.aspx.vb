Partial Class GF_costProjectsIputApproval
  ...
  End Sub
  Protected Sub GVcostProjectsIputApproval_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVcostProjectsIputApproval.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ProjectGroupID As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("ProjectGroupID")  
        Dim FinYear As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("FinYear")  
        Dim Quarter As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("Quarter")  
        Dim RedirectUrl As String = TBLcostProjectsIputApproval.EditUrl & "?ProjectGroupID=" & ProjectGroupID & "&FinYear=" & FinYear & "&Quarter=" & Quarter
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVcostProjectsIputApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
        Dim ProjectGroupID As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("ProjectGroupID")  
        Dim FinYear As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("FinYear")  
        Dim Quarter As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("Quarter")  
        SIS.COST.costProjectsIputApproval.ApproveWF(ProjectGroupID, FinYear, Quarter, ApproverRemarks)
        GVcostProjectsIputApproval.DataBind()
      Catch ex As Exception
      End Try
    End If
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        ...
        Dim FinYear As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("FinYear")  
        Dim Quarter As Int32 = GVcostProjectsIputApproval.DataKeys(e.CommandArgument).Values("Quarter")  
        SIS.COST.costProjectsIputApproval.RejectWF(ProjectGroupID, FinYear, Quarter, ApproverRemarks)
        GVcostProjectsIputApproval.DataBind()
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVcostProjectsIputApproval_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVcostProjectsIputApproval.Init
    DataClassName = "GcostProjectsIputApproval"
    SetGridView = GVcostProjectsIputApproval
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim validateScriptProjectGroupID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_ProjectsInput_ProjectGroupID_main = true;" & _
      "    validate_FK_COST_ProjectsInput_ProjectGroupID(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptFinYear As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_ProjectsInput_FinYear_main = true;" & _
      "    validate_FK_COST_ProjectsInput_FinYear(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptQuarter As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_COST_ProjectsInput_Quarter_main = true;" & _
      "    validate_FK_COST_ProjectsInput_Quarter(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_COST_ProjectsInput_FinYear As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_ProjectsInput_FinYear(o) {" & _
      "    var value = o.id;" & _
      "    var FinYear = $get('" & F_FinYear.ClientID & "');" & _
      "    try{" & _
      "    if(FinYear.value==''){" & _
      "      if(validated_FK_COST_ProjectsInput_FinYear.main){" & _
      ...
      "    PageMethods.validate_FK_COST_ProjectsInput_FinYear(value, validated_FK_COST_ProjectsInput_FinYear);" & _
      "  }" & _
      "  validated_FK_COST_ProjectsInput_FinYear_main = false;" & _
      "  function validated_FK_COST_ProjectsInput_FinYear(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_ProjectsInput_FinYear") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_ProjectsInput_FinYear", validateScriptFK_COST_ProjectsInput_FinYear)
    End If
    Dim validateScriptFK_COST_ProjectsInput_ProjectGroupID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_ProjectsInput_ProjectGroupID(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectGroupID = $get('" & F_ProjectGroupID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectGroupID.value==''){" & _
      "      if(validated_FK_COST_ProjectsInput_ProjectGroupID.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_ProjectsInput_ProjectGroupID(value, validated_FK_COST_ProjectsInput_ProjectGroupID);" & _
      "  }" & _
      "  validated_FK_COST_ProjectsInput_ProjectGroupID_main = false;" & _
      "  function validated_FK_COST_ProjectsInput_ProjectGroupID(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_ProjectsInput_ProjectGroupID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_ProjectsInput_ProjectGroupID", validateScriptFK_COST_ProjectsInput_ProjectGroupID)
    End If
    Dim validateScriptFK_COST_ProjectsInput_Quarter As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_COST_ProjectsInput_Quarter(o) {" & _
      "    var value = o.id;" & _
      "    var Quarter = $get('" & F_Quarter.ClientID & "');" & _
      "    try{" & _
      "    if(Quarter.value==''){" & _
      "      if(validated_FK_COST_ProjectsInput_Quarter.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_COST_ProjectsInput_Quarter(value, validated_FK_COST_ProjectsInput_Quarter);" & _
      "  }" & _
      "  validated_FK_COST_ProjectsInput_Quarter_main = false;" & _
      "  function validated_FK_COST_ProjectsInput_Quarter(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_COST_ProjectsInput_Quarter") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_COST_ProjectsInput_Quarter", validateScriptFK_COST_ProjectsInput_Quarter)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
