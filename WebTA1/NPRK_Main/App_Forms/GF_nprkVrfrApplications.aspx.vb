...
Partial Class GF_nprkVrfrApplications
  Inherits SIS.SYS.GridBase
  Protected Sub GVnprkVrfrApplications_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVnprkVrfrApplications.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ClaimID As Int32 = GVnprkVrfrApplications.DataKeys(e.CommandArgument).Values("ClaimID")
        Dim ApplicationID As Int32 = GVnprkVrfrApplications.DataKeys(e.CommandArgument).Values("ApplicationID")
        Dim RedirectUrl As String = TBLnprkVrfrApplications.EditUrl & "?ClaimID=" & ClaimID & "&ApplicationID=" & ApplicationID
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim Documents As Boolean = CType(GVnprkVrfrApplications.Rows(e.CommandArgument).FindControl("F_Documents"), DropDownList).SelectedValue
        Dim Verified As Boolean = CType(GVnprkVrfrApplications.Rows(e.CommandArgument).FindControl("F_Verified"), DropDownList).SelectedValue
        Dim PaymentMethod As String = CType(GVnprkVrfrApplications.Rows(e.CommandArgument).FindControl("F_PaymentMethod"), DropDownList).SelectedValue
        Dim VerifiedAmt As Decimal = CType(GVnprkVrfrApplications.Rows(e.CommandArgument).FindControl("F_VerifiedAmt"), TextBox).Text
        Dim VerifierRemark As String = CType(GVnprkVrfrApplications.Rows(e.CommandArgument).FindControl("F_VerifierRemark"), TextBox).Text
        Dim ClaimID As Int32 = GVnprkVrfrApplications.DataKeys(e.CommandArgument).Values("ClaimID")
        Dim ApplicationID As Int32 = GVnprkVrfrApplications.DataKeys(e.CommandArgument).Values("ApplicationID")
        ...
        GVnprkVrfrApplications.DataBind()
      Catch ex As Exception
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        ...
        Dim ClaimID As Int32 = GVnprkVrfrApplications.DataKeys(e.CommandArgument).Values("ClaimID")
        Dim ApplicationID As Int32 = GVnprkVrfrApplications.DataKeys(e.CommandArgument).Values("ApplicationID")
        SIS.NPRK.nprkVrfrApplications.RejectWF(ClaimID, ApplicationID, Documents, Verified, PaymentMethod, VerifiedAmt, VerifierRemark)
        GVnprkVrfrApplications.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
    ...
    End If
  End Sub
  Protected Sub GVnprkVrfrApplications_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVnprkVrfrApplications.Init
    DataClassName = "GnprkVrfrApplications"
    SetGridView = GVnprkVrfrApplications
  End Sub
  ...
  Private Sub UPNLnprkVrfrApplications_DataBinding(sender As Object, e As EventArgs) Handles UPNLnprkVrfrApplications.DataBinding
    GVnprkVrfrApplications.DataBind()
  End Sub

  Private Sub lc_yeardate1_YearDateChanged(sender As Object, e As EventArgs) Handles lc_yeardate1.YearDateChanged
    UPNLnprkVrfrApplications.DataBind()
  End Sub
