...
Partial Class GF_nprkCashPayment
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/NPRK_Main/App_Display/DF_nprkCashPayment.aspx"
  ...
  End Sub
  Protected Sub GVnprkCashPayment_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVnprkCashPayment.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim ClaimID As Int32 = GVnprkCashPayment.DataKeys(e.CommandArgument).Values("ClaimID")
        Dim ApplicationID As Int32 = GVnprkCashPayment.DataKeys(e.CommandArgument).Values("ApplicationID")
        Dim RedirectUrl As String = TBLnprkCashPayment.EditUrl & "?ClaimID=" & ClaimID & "&ApplicationID=" & ApplicationID
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim UpdatedInLedger As Boolean = CType(GVnprkCashPayment.Rows(e.CommandArgument).FindControl("F_UpdatedInLedger"), DropDownList).SelectedValue
        Dim PaidAt As String = CType(GVnprkCashPayment.Rows(e.CommandArgument).FindControl("F_PaidAt"), DropDownList).SelectedValue
        Dim ClaimID As Int32 = GVnprkCashPayment.DataKeys(e.CommandArgument).Values("ClaimID")
        Dim ApplicationID As Int32 = GVnprkCashPayment.DataKeys(e.CommandArgument).Values("ApplicationID")
        SIS.NPRK.nprkCashPayment.ApproveWF(ClaimID, ApplicationID, UpdatedInLedger, PaidAt)
        GVnprkCashPayment.DataBind()
      Catch ex As Exception
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        ...
        Dim ClaimID As Int32 = GVnprkCashPayment.DataKeys(e.CommandArgument).Values("ClaimID")
        Dim ApplicationID As Int32 = GVnprkCashPayment.DataKeys(e.CommandArgument).Values("ApplicationID")
        SIS.NPRK.nprkCashPayment.RejectWF(ClaimID, ApplicationID, UpdatedInLedger, PaidAt)
        GVnprkCashPayment.DataBind()
      Catch ex As Exception
        Dim message As String = New JavaScriptSerializer().Serialize(ex.Message.ToString())
        Dim script As String = String.Format("alert({0});", message)
    ...
    End If
  End Sub
  Protected Sub GVnprkCashPayment_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVnprkCashPayment.Init
    DataClassName = "GnprkCashPayment"
    SetGridView = GVnprkCashPayment
  End Sub
  ...
  Private Sub UPNLnprkCashPayment_DataBinding(sender As Object, e As EventArgs) Handles UPNLnprkCashPayment.DataBinding
    GVnprkCashPayment.DataBind()
  End Sub

  Private Sub lc_yeardate1_YearDateChanged(s As Object, e As EventArgs) Handles lc_yeardate1.YearDateChanged
    UPNLnprkCashPayment.DataBind()
  End Sub
End Class
