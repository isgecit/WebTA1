...
Partial Class GF_taBHAll
  ...
  Protected Sub GVtaBH_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVtaBH.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      ...
      End Try
    End If
    If e.CommandName.ToLower = "lgempedit".ToLower Then
      Try
        Dim TABillNo As Int32 = GVtaBH.DataKeys(e.CommandArgument).Values("TABillNo")
        Dim CardNo As String = CType(GVtaBH.Rows(e.CommandArgument).FindControl("L_EmployeeID"), LinkButton).ToolTip
        Dim RedirectUrl As String = "~/TA_Main/App_Edit/EF_taEmployees.aspx" & "?CardNo=" & CardNo & "&TABillNo=" & TABillNo
        Response.Redirect(RedirectUrl)
      Catch ex As Exception
      End Try
    End If
    If e.CommandName.ToLower = "lgstatussave".ToLower Then
      Try
        Dim TABillNo As Int32 = GVtaBH.DataKeys(e.CommandArgument).Values("TABillNo")
        Dim StatusID As Integer = CType(GVtaBH.Rows(e.CommandArgument).FindControl("LC_BillStatusID"), LC_taBillStates).SelectedValue
        Dim oBil As SIS.TA.taBH = SIS.TA.taBH.taBHGetByID(TABillNo)
        oBil.BillStatusID = StatusID
        SIS.TA.taBH.UpdateData(oBil)
        GVtaBH.DataBind()
      Catch ex As Exception
      End Try
    End If
    ...
    If e.CommandName.ToLower = "lgdelete".ToLower Then
      Try
        ...
        Dim StatusID As Integer = CType(GVtaBH.Rows(e.CommandArgument).FindControl("LC_BillStatusID"), LC_taBillStates).SelectedValue
        If StatusID <> 1 Then
          Dim str As String = New JavaScriptSerializer().Serialize("Status ID is NOT FREE, Can Not Delete")
          Dim script As String = String.Format("alert({0});", str)
          ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", script, True)
          Exit Sub
        End If
        Dim prj As List(Of SIS.TA.taBillPrjAmounts) = SIS.TA.taBillPrjAmounts.taBillPrjAmountsSelectList(0, 999, "", False, "", TABillNo)
        For Each tmp As SIS.TA.taBillPrjAmounts In prj
          SIS.TA.taBillPrjAmounts.taBillPrjAmountsDelete(tmp)
        Next
        Dim amt As List(Of SIS.TA.taBillAmount) = SIS.TA.taBillAmount.taBillAmountSelectList(0, 999, "", False, "", TABillNo)
        For Each tmp As SIS.TA.taBillAmount In amt
          SIS.TA.taBillAmount.taBillAmountDelete(tmp)
        Next
        Dim det As List(Of SIS.TA.taBillDetails) = SIS.TA.taBillDetails.taBillDetailsSelectList(0, 999, "", False, "", TABillNo)
        For Each tmp As SIS.TA.taBillDetails In det
          If tmp.ComponentID = TAComponentTypes.Lodging Then
            Dim oGST As SIS.TA.taBillGST = SIS.TA.taBillGST.taBillGSTGetByID(tmp.TABillNo, tmp.SerialNo)
            SIS.TA.taBillGST.taBillGSTDelete(oGST)
          End If
          SIS.TA.taBillDetails.taBillDetailsDelete(tmp)
        Next
        Dim oBil As SIS.TA.taBH = SIS.TA.taBH.taBHGetByID(TABillNo)
        SIS.TA.taBH.taBHDelete(oBil)
        GVtaBH.DataBind()
      Catch ex As Exception
      End Try
    End If
