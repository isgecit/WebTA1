...
Partial Class GF_tarTRSanction
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/TAR_Main/App_Display/DF_tarTRSanction.aspx"
  ...
  End Sub
  Protected Sub GVtarTRSanction_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVtarTRSanction.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim RequestID As Int32 = GVtarTRSanction.DataKeys(e.CommandArgument).Values("RequestID")  
        Dim RedirectUrl As String = TBLtarTRSanction.EditUrl & "?RequestID=" & RequestID
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "Budgetwf".ToLower Then
      Try
        Dim BHRemarks As String = CType(GVtarTRSanction.Rows(e.CommandArgument).FindControl("F_BHRemarks"),TextBox).Text
        Dim RequestID As Int32 = GVtarTRSanction.DataKeys(e.CommandArgument).Values("RequestID")  
        SIS.TAR.tarTRSanction.BudgetWF(RequestID, BHRemarks)
        GVtarTRSanction.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim BHRemarks As String = CType(GVtarTRSanction.Rows(e.CommandArgument).FindControl("F_BHRemarks"),TextBox).Text
        Dim RequestID As Int32 = GVtarTRSanction.DataKeys(e.CommandArgument).Values("RequestID")  
        SIS.TAR.tarTRSanction.ApproveWF(RequestID, BHRemarks)
        GVtarTRSanction.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim BHRemarks As String = CType(GVtarTRSanction.Rows(e.CommandArgument).FindControl("F_BHRemarks"),TextBox).Text
        Dim RequestID As Int32 = GVtarTRSanction.DataKeys(e.CommandArgument).Values("RequestID")  
        SIS.TAR.tarTRSanction.RejectWF(RequestID, BHRemarks)
        GVtarTRSanction.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVtarTRSanction_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVtarTRSanction.Init
    DataClassName = "GtarTRSanction"
    SetGridView = GVtarTRSanction
  End Sub
