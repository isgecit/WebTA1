...
Partial Class GF_tarTRApproval
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/TAR_Main/App_Display/DF_tarTRApproval.aspx"
  ...
  End Sub
  Protected Sub GVtarTRApproval_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVtarTRApproval.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim RequestID As Int32 = GVtarTRApproval.DataKeys(e.CommandArgument).Values("RequestID")  
        Dim RedirectUrl As String = TBLtarTRApproval.EditUrl & "?RequestID=" & RequestID
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "Budgetwf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVtarTRApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
        Dim RequestID As Int32 = GVtarTRApproval.DataKeys(e.CommandArgument).Values("RequestID")  
        SIS.TAR.tarTRApproval.BudgetWF(RequestID, ApproverRemarks)
        GVtarTRApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVtarTRApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
        Dim RequestID As Int32 = GVtarTRApproval.DataKeys(e.CommandArgument).Values("RequestID")  
        SIS.TAR.tarTRApproval.ApproveWF(RequestID, ApproverRemarks)
        GVtarTRApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim ApproverRemarks As String = CType(GVtarTRApproval.Rows(e.CommandArgument).FindControl("F_ApproverRemarks"),TextBox).Text
        Dim RequestID As Int32 = GVtarTRApproval.DataKeys(e.CommandArgument).Values("RequestID")  
        SIS.TAR.tarTRApproval.RejectWF(RequestID, ApproverRemarks)
        GVtarTRApproval.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVtarTRApproval_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVtarTRApproval.Init
    DataClassName = "GtarTRApproval"
    SetGridView = GVtarTRApproval
  End Sub
