Partial Class GF_psfApproval
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/PSF_Main/App_Display/DF_psfApproval.aspx"
  ...
  End Sub
  Protected Sub GVpsfApproval_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpsfApproval.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpsfApproval.DataKeys(e.CommandArgument).Values("SerialNo")  
        Dim RedirectUrl As String = TBLpsfApproval.EditUrl & "?SerialNo=" & SerialNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpsfApproval.DataKeys(e.CommandArgument).Values("SerialNo")  
        SIS.PSF.psfApproval.ApproveWF(SerialNo)
        GVpsfApproval.DataBind()
      Catch ex As Exception
      End Try
    End If
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpsfApproval.DataKeys(e.CommandArgument).Values("SerialNo")  
        SIS.PSF.psfApproval.RejectWF(SerialNo)
        GVpsfApproval.DataBind()
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVpsfApproval_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpsfApproval.Init
    DataClassName = "GpsfApproval"
    SetGridView = GVpsfApproval
  End Sub
