...
var script_elogDetentionBill = {
    ...
    validate_IRNo: function(sender) {
      ...
      this.validatePK_elogDetentionBill(sender,Prefix);
      },
    ...
    validatePK_elogDetentionBill: function(o,Prefix) {
      var value = o.id;
      try{$get(Prefix.replace('_F_','') + '_L_ErrMsgelogDetentionBill').innerHTML = '';}catch(ex){}
      ...
      o.style.backgroundRepeat= 'no-repeat';
      o.style.backgroundPosition = 'right';
      PageMethods.validatePK_elogDetentionBill(value, this.validatedPK_elogDetentionBill);
    },
    validatedPK_elogDetentionBill: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      o.style.backgroundImage  = 'none';
      if(p[0]=='1'){
        try{$get('cph1_FVelogDetentionBill_L_ErrMsgelogDetentionBill').innerHTML = p[2];}catch(ex){}
        o.value='';
        o.focus();
      }
    },
...
var script_vrTransporterBill = {
  ...
  validatedPK_vrTransporterBill: function(result) {
    ...
    o.style.backgroundImage  = 'none';
    if(p[0]=='1'){
      try{$get('cph1_FVvrTransporterBill_L_ErrMsgvrTransporterBill').innerHTML = p[2];}catch(ex){}
      o.value='';
      o.focus();
    }else{
