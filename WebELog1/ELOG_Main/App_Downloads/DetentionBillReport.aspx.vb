...
Partial Class DetentionBillReport
  ...
  Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Dim DWFile As String = "DetentionBill Report"
  ...
  Private Function CreateFile(ByVal project As String) As String
    ...
    IO.File.Copy(Server.MapPath("~/App_Templates") & "\DetentionBillTemplate.xlsx", FileName)
    Dim FileInfo As IO.FileInfo = New IO.FileInfo(FileName)
    Dim xlPk As ExcelPackage = New ExcelPackage(FileInfo)

    Dim xlWS As ExcelWorksheet = xlPk.Workbook.Worksheets("DetentionBill_DATA")
    Dim oDocs As List(Of DetentionBillReportClass) = DetentionBillReportClass.GetData(project)
    Dim r As Integer = 5
    Dim c As Integer = 2
    Dim s As Integer = 1
    ...
    With xlWS
      For Each doc As DetentionBillReportClass In oDocs
        If r > 5 Then
  ...
  End Function
End Class
Public Class DetentionBillReportClass
  ...
  Public Sub New()
  End Sub
  Public Shared Function GetData(ByVal project As String) As List(Of DetentionBillReportClass)
    ...
    Return GetDetentionBillReportClass(Sql)
  End Function

  Private Shared Function GetDetentionBillReportClass(ByVal Sql As String) As List(Of DetentionBillReportClass)
    Dim Results As List(Of DetentionBillReportClass) = Nothing
    Using Con As SqlConnection = New SqlConnection(SIS.SYS.SQLDatabase.DBCommon.GetConnectionString())
      Using Cmd As SqlCommand = Con.CreateCommand()
        Cmd.CommandType = CommandType.Text
        Cmd.CommandText = Sql
        Cmd.CommandTimeout = 1200
        Results = New List(Of DetentionBillReportClass)
        Con.Open()
        Dim Reader As SqlDataReader = Cmd.ExecuteReader()
        While (Reader.Read())
          Results.Add(New DetentionBillReportClass(Reader))
        End While
        Reader.Close()
      End Using
    End Using
