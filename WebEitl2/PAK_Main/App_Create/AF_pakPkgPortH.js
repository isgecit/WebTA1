...
var script_pakPkgListH = {
    ...
    validate_ProjectID: function(sender) {
      ...
      this.validated_FK_PAK_PkgListH_ProjectID_main = true;
      this.validate_FK_PAK_PkgListH_ProjectID(sender,Prefix);
    },
    validate_FK_PAK_PkgListH_ProjectID: function(o,Prefix) {
      var value = o.id;
      var ProjectID = $get(Prefix + 'ProjectID');
      if(ProjectID.value==''){
        if(this.validated_FK_PAK_PkgListH_ProjectID_main){
      ...
      o.style.backgroundRepeat= 'no-repeat';
      o.style.backgroundPosition = 'right';
      PageMethods.validate_FK_PAK_PkgListH_ProjectID(value, this.validated_FK_PAK_PkgListH_ProjectID);
    },
    validated_FK_PAK_PkgListH_ProjectID_main: false,
    validated_FK_PAK_PkgListH_ProjectID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pakPkgListH.validated_FK_PAK_PkgListH_ProjectID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
