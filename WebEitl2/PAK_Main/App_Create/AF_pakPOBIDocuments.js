...
var script_pakPOBIDocuments = {
    ...
    validated_FK_PAK_POBIDocuments_SerialNo: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pakPOBIDocuments.validated_FK_PAK_POBIDocuments_SerialNo_main){
    ...
    validated_FK_PAK_POBIDocuments_ItemNo: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pakPOBIDocuments.validated_FK_PAK_POBIDocuments_ItemNo_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
    ...
    validated_FK_PAK_POBIDocuments_BOMNo: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pakPOBIDocuments.validated_FK_PAK_POBIDocuments_BOMNo_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
