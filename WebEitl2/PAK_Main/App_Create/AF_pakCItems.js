...
var script_pakCItems = {
    ...
    validate_RootItem: function(sender) {
      var Prefix = sender.id.replace('RootItem','');
      this.validated_FK_PAK_Items_RootItem_main = true;
      this.validate_FK_PAK_Items_RootItem(sender,Prefix);
      },
    ...
    validate_FK_PAK_Items_RootItem: function(o,Prefix) {
      var value = o.id;
      var RootItem = $get(Prefix + 'RootItem');
      if(RootItem.value==''){
        if(this.validated_FK_PAK_Items_RootItem_main){
          var o_d = $get(Prefix + 'RootItem' + '_Display');
          try{o_d.innerHTML = '';}catch(ex){}
        }
        return true;
      }
      value = value + ',' + RootItem.value ;
        ...
        o.style.backgroundRepeat= 'no-repeat';
        o.style.backgroundPosition = 'right';
        PageMethods.validate_FK_PAK_Items_RootItem(value, this.validated_FK_PAK_Items_RootItem);
      },
    validated_FK_PAK_Items_RootItem_main: false,
    validated_FK_PAK_Items_RootItem: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_pakCItems.validated_FK_PAK_Items_RootItem_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
