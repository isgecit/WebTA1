...
Partial Class GF_pakIQCListD
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/PAK_Main/App_Display/DF_pakIQCListD.aspx"
  Protected Sub Info_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
    Dim oBut As ImageButton = CType(sender, ImageButton)
    Dim aVal() As String = oBut.CommandArgument.ToString.Split(",".ToCharArray)
    Dim RedirectUrl As String = _InfoUrl  & "?SerialNo=" & aVal(0) & "&QCLNo=" & aVal(1) & "&BOMNo=" & aVal(2) & "&ItemNo=" & aVal(3)
  ...
  Protected Sub GVpakIQCListD_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpakIQCListD.Init
    DataClassName = "GpakIQCListD"
    SetGridView = GVpakIQCListD
  End Sub
  ...
  Protected Sub F_QCLNo_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles F_QCLNo.TextChanged
    ...
    Session("F_QCLNo_Display") = F_QCLNo_Display.Text
    InitGridPage()
  End Sub
  <System.Web.Services.WebMethod()> _
  <System.Web.Script.Services.ScriptMethod()> _
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim strScriptPopulatingSerialNo As String = "<script type=""text/javascript""> " & _
      ...
      End If
    F_QCLNo_Display.Text = String.Empty
    If Not Session("F_QCLNo_Display") Is Nothing Then
      If Session("F_QCLNo_Display") <> String.Empty Then
        F_QCLNo_Display.Text = Session("F_QCLNo_Display")
      End If
    End If
    F_QCLNo.Text = String.Empty
    If Not Session("F_QCLNo") Is Nothing Then
      If Session("F_QCLNo") <> String.Empty Then
        F_QCLNo.Text = Session("F_QCLNo")
      End If
    End If
    Dim strScriptQCLNo As String = "<script type=""text/javascript""> " & _
      "function ACEQCLNo_Selected(sender, e) {" & _
      "  var F_QCLNo = $get('" & F_QCLNo.ClientID & "');" & _
      "  var F_QCLNo_Display = $get('" & F_QCLNo_Display.ClientID & "');" & _
      "  var retval = e.get_value();" & _
      "  var p = retval.split('|');" & _
      "  F_QCLNo.value = p[1];" & _
      "  F_QCLNo_Display.innerHTML = e.get_text();" & _
      "}" & _
      "</script>"
      If Not Page.ClientScript.IsClientScriptBlockRegistered("F_QCLNo") Then
        Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "F_QCLNo", strScriptQCLNo)
      End If
    Dim strScriptPopulatingQCLNo As String = "<script type=""text/javascript""> " & _
      "function ACEQCLNo_Populating(o,e) {" & _
      "  var p = $get('" & F_QCLNo.ClientID & "');" & _
      "  p.style.backgroundImage  = 'url(../../images/loader.gif)';" & _
      "  p.style.backgroundRepeat= 'no-repeat';" & _
      "  p.style.backgroundPosition = 'right';" & _
      "  o._contextKey = '';" & _
      "}" & _
      "function ACEQCLNo_Populated(o,e) {" & _
      "  var p = $get('" & F_QCLNo.ClientID & "');" & _
      "  p.style.backgroundImage  = 'none';" & _
      "}" & _
      "</script>"
      If Not Page.ClientScript.IsClientScriptBlockRegistered("F_QCLNoPopulating") Then
        Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "F_QCLNoPopulating", strScriptPopulatingQCLNo)
      End If
    Dim validateScriptSerialNo As String = "<script type=""text/javascript"">" & _
      "  function validate_SerialNo(o) {" & _
      "    validated_FK_PAK_QCListD_SerialNo_main = true;" & _
      "    validate_FK_PAK_QCListD_SerialNo(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptQCLNo As String = "<script type=""text/javascript"">" & _
      "  function validate_QCLNo(o) {" & _
      "    validated_FK_PAK_QCListD_QCLNo_main = true;" & _
      "    validate_FK_PAK_QCListD_QCLNo(o);" & _
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateQCLNo") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateQCLNo", validateScriptQCLNo)
    End If
    Dim validateScriptFK_PAK_QCListD_SerialNo As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_PAK_QCListD_SerialNo(o) {" & _
      "    var value = o.id;" & _
      "    var SerialNo = $get('" & F_SerialNo.ClientID & "');" & _
      "    try{" & _
      "    if(SerialNo.value==''){" & _
      "      if(validated_FK_PAK_QCListD_SerialNo.main){" & _
      ...
      "    PageMethods.validate_FK_PAK_QCListD_SerialNo(value, validated_FK_PAK_QCListD_SerialNo);" & _
      "  }" & _
      "  validated_FK_PAK_QCListD_SerialNo_main = false;" & _
      "  function validated_FK_PAK_QCListD_SerialNo(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_PAK_QCListD_SerialNo") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_PAK_QCListD_SerialNo", validateScriptFK_PAK_QCListD_SerialNo)
    End If
    Dim validateScriptFK_PAK_QCListD_QCLNo As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_PAK_QCListD_QCLNo(o) {" & _
      "    var value = o.id;" & _
      "    var SerialNo = $get('" & F_SerialNo.ClientID & "');" & _
      "    try{" & _
      "    if(SerialNo.value==''){" & _
      "      if(validated_FK_PAK_QCListD_QCLNo.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      "    value = value + ',' + SerialNo.value ;" & _
      "    }catch(ex){}" & _
      "    var QCLNo = $get('" & F_QCLNo.ClientID & "');" & _
      "    try{" & _
      "    if(QCLNo.value==''){" & _
      "      if(validated_FK_PAK_QCListD_QCLNo.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      ...
      "      }" & _
      "    }" & _
      "    value = value + ',' + QCLNo.value ;" & _
      "    }catch(ex){}" & _
      "    o.style.backgroundImage  = 'url(../../images/pkloader.gif)';" & _
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_PAK_QCListD_QCLNo(value, validated_FK_PAK_QCListD_QCLNo);" & _
      "  }" & _
      "  validated_FK_PAK_QCListD_QCLNo_main = false;" & _
      "  function validated_FK_PAK_QCListD_QCLNo(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_PAK_QCListD_QCLNo") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_PAK_QCListD_QCLNo", validateScriptFK_PAK_QCListD_QCLNo)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
