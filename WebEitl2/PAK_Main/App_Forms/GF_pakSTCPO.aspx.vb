...
Partial Class GF_pakSTCPO
  ...
  End Sub
  Protected Sub GVpakSTCPO_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVpakSTCPO.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSTCPO.DataKeys(e.CommandArgument).Values("SerialNo")  
        Dim RedirectUrl As String = TBLpakSTCPO.EditUrl & "?SerialNo=" & SerialNo
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "approvewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSTCPO.DataKeys(e.CommandArgument).Values("SerialNo")  
        SIS.PAK.pakSTCPO.ApproveWF(SerialNo)
        GVpakSTCPO.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    ...
    If e.CommandName.ToLower = "completewf".ToLower Then
      Try
        Dim SerialNo As Int32 = GVpakSTCPO.DataKeys(e.CommandArgument).Values("SerialNo")  
        SIS.PAK.pakSTCPO.CompleteWF(SerialNo)
        GVpakSTCPO.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVpakSTCPO_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVpakSTCPO.Init
    DataClassName = "GpakSTCPO"
    SetGridView = GVpakSTCPO
  End Sub
