Partial Class GF_lgEPFile
  ...
  Protected Sub GVlgEPFile_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVlgEPFile.Init
    DataClassName = "GlgEPFile"
    SetGridView = GVlgEPFile
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    End If
		...
		Dim validateScriptDocPK As String = "<script type=""text/javascript"">" & _
			...
			"    validated_FK_LG_EPFile_DocPK_main = true;" & _
			"    validate_FK_LG_EPFile_DocPK(o);" & _
			"  }" & _
		  "</script>"
		...
		End If
		Dim validateScriptFK_LG_EPFile_DocPK As String = "<script type=""text/javascript"">" & _
			"  function validate_FK_LG_EPFile_DocPK(o) {" & _
			"    var value = o.id;" & _
			"    var DocPK = $get('" & F_DocPK.ClientID & "');" & _
			"    try{" & _
			"    if(DocPK.value==''){" & _
			"      if(validated_FK_LG_EPFile_DocPK.main){" & _
			...
			"    PageMethods.validate_FK_LG_EPFile_DocPK(value, validated_FK_LG_EPFile_DocPK);" & _
			"  }" & _
			"  validated_FK_LG_EPFile_DocPK_main = false;" & _
			"  function validated_FK_LG_EPFile_DocPK(result) {" & _
			...
			"  }" & _
		  "</script>"
		If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_LG_EPFile_DocPK") Then
			Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_LG_EPFile_DocPK", validateScriptFK_LG_EPFile_DocPK)
		End If
  End Sub
	<System.Web.Services.WebMethod()> _
