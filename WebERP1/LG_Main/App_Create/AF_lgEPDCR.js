...
var script_lgEPDCR = {
		...
		validate_DocPK: function(sender) {
		  ...
		  this.validated_FK_LG_EPDCR_DocPK_main = true;
		  this.validate_FK_LG_EPDCR_DocPK(sender,Prefix);
		  },
		validate_FK_LG_EPDCR_DocPK: function(o,Prefix) {
		  var value = o.id;
		  var DocPK = $get(Prefix + 'DocPK');
		  if(DocPK.value==''){
		    if(this.validated_FK_LG_EPDCR_DocPK_main){
		  ...
		  value = value + ',' + DocPK.value ;
		    ...
		    PageMethods.validate_FK_LG_EPDCR_DocPK(value, this.validated_FK_LG_EPDCR_DocPK);
		  },
		validated_FK_LG_EPDCR_DocPK_main: false,
		validated_FK_LG_EPDCR_DocPK: function(result) {
		  var p = result.split('|');
		  var o = $get(p[1]);
		  if(script_lgEPDCR.validated_FK_LG_EPDCR_DocPK_main){
